...
def callback2(message):
    ...
    message = message.joint_state
    position = message.position
    theta = np.array(np.hstack([position[1:8]]))
    g = task1(theta)
    print(g)
...
def query():
    ...
    while not rospy.is_shutdown():
        X = float(raw_input('Enter an X value:'))
        Y = float(raw_input('Enter an Y value:'))
        Z = float(raw_input('Enter an Z value:'))
